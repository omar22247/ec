...
public class OrderController {
    ...
    public ResponseEntity<ApiResponse<OrderResponse>> createOrder(
        ...
        return ResponseEntity
                ...
                .body(ApiResponse.success("Order placed successfully",
                        orderService.createOrder(userId(userDetails), request)));
    }
    ...
    public ResponseEntity<ApiResponse<PageResponse<OrderSummaryResponse>>> getMyOrders(
        ...
        return ResponseEntity.ok(
                ApiResponse.success(
                        orderService.getMyOrders(userId(userDetails), page, size))
        );
    }

    @GetMapping("/api/v1/orders/{id}")
    @Operation(summary = "Get order details",
            description = "Retrieves full details of a specific order for the authenticated user")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200",
    ...
    public ResponseEntity<ApiResponse<OrderResponse>> getOrderById(
        ...
        return ResponseEntity.ok(
                ApiResponse.success(
                        orderService.getOrderById(userId(userDetails), id))
        );
    }
